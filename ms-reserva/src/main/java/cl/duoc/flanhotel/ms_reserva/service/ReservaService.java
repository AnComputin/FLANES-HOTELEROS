    ...
    public void eliminarReserva(Long id) {
        Reserva reserva = reservaRepository.findById(id)
                .orElseThrow(() -> new RuntimeException(
                        "No se puede eliminar. Reserva no encontrada con el ID: " + id));

        reservaRepository.deleteById(id);

        try {
            habitacionFeignClient.actualizarEstadoHabitacion(reserva.getIdHabitacion(), "DISPONIBLE");
            log.info("Habitación ID {} liberada al eliminar reserva ID {}", reserva.getIdHabitacion(), id);
        } catch (Exception e) {
            log.error("No se pudo liberar la habitación al eliminar reserva: {}", e.getMessage());
        }
    }
